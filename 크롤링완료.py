...
'''   
############
#### 영국남자, 지현꿍 모든 댓글 열기 성공, 안한거 : 채널 검색해서 하나하나 치는거, 댓글 저장하기,  엑셀에 저장
############
...
driver.get("https://www.youtube.com/user/koreanenglishman/community")

#멤버십 구독 버튼 클릭
member_button = driver.find_elements_by_css_selector("yt-formatted-string.style-scope.yt-button-renderer.style-blue-text.size-default")
...
qna_cont = driver.find_elements_by_xpath("//*[contains(text(), 'QnA') or contains(text(), 'Q&A') or contains(text(), '큐엔에이') or contains(text(), '궁금한 점') or contains(text(), 'q&a') or contains(text(), 'Q/A')]")
...
for qna_content in qna_can_cont:
    ...
    #if qna_com == []:
    #    continue #댓글 모두보기가 아니라 새창 댓글로 넘어가는 경우.. 비어있게 됨. 따라서 밑 코드 생략
    ###그 게시글의 댓글로 이동해서 클릭
...
driver.get("https://www.youtube.com/channel/UCX3QAQPW30ZPpj39LJjJ_3g/community")
...
qna_cont = driver.find_elements_by_xpath("//*[contains(text(), 'QnA') or contains(text(), 'Q&A') or contains(text(), '큐엔에이') or contains(text(), '궁금한 점') or contains(text(), 'q&a') or contains(text(), 'Q/A') or contains(text(), '궁금하신') or contains(text(), '질문하나')]")
qna_can_cont = interact(qna_cont)
...
for comcont in container:
    ...
    # ["전체 인덱스","채널명","채널인덱스","댓글내용","좋아요","실제 좋아요"]
    #sheet.append([index, ch_index, comment, like, like_text])
# print("==================")
...
###############
###커뮤니티 바로 접속해서 댓글 모두보기 끝까지해서 읽고 새창이동해서 끝까지 읽는거까지완료
###############
...
ch = pd.read_csv("channel_qna.csv")
try:
    wb = openpyxl.load_workbook("result.xlsx")
except:
    wb = openpyxl.Workbook()
    sheet = wb.active
    sheet.append(["추가일시", "전체 인덱스", "채널명", "채널인덱스", "글구분", "좋아요", "실제좋아요"])
else:
    sheet = wb.active
